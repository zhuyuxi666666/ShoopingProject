...
public class DialogMaker {
	 ...
	 * @return 显示并返回对话框
	 */
	public static Dialog showCommonAlertDialog(Context context, String title, String msg,
											   String[] btns, final DialogCallBack callBack,
											   boolean isCanCancelabel,
											   final boolean isDismissAfterClickBtn,
											   final Object tag) {
		final Dialog dialog = new Dialog(context, R.style.DialogNoTitleStyleTranslucentBg);
		View contentView = LayoutInflater.from(context)
				.inflate(R.layout.dialog_alert_common_layout, null);
		...
		if (null != btns) {
			...
			if (len > 0) {
				...
				for (int i = 0; i < len; i++) {
					...
					if (i < len - 1 && len > 1) {
						btnLayout.addView(LineView, new LinearLayout.LayoutParams(1,
		...
		aWmLp.width = MyApplication.getInstance().screenW - 100;
	 ...
	 *
	 */
	public static Dialog showCommenWaitDialog(Context context, String msg,
											  final DialogCallBack callBack,
											  boolean isCanCancelabel, final Object tag) {
		final Dialog dialog = new Dialog(context, R.style.DialogNoTitleRoundCornerStyle);
		dialog.setOwnerActivity((Activity)context);
		View contentView = LayoutInflater.from(context)
				.inflate(R.layout.dialog_wait_common_layout, null);
		TextView contentTv = (TextView) contentView.findViewById(R.id.dialog_content_tv);
		if (JStringKit.isNotBlank(msg)) {
